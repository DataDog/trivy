// Should be synchronized with github.com/aquasecurity/trivy/pkg/fanal/artifact/image/image.go.
...
func (a Artifact) calcCacheKeys(imageID string, diffIDs []string) (string, []string, error) {
	...
	imageKey, err := cache.CalcKey(imageID, a.analyzer.AnalyzerVersions(), nil, artifact.Option{}) // XXX a.configAnalyzer.AnalyzerVersions()
	if err != nil {
		return "", nil, err
	}
...
func (a Artifact) inspectLayer(ctx context.Context, layerInfo LayerInfo, disabled []analyzer.Type) (types.BlobInfo, error) {
	...
	a.logger.Info("Walk Layer", log.String("path", layerInfo.Path))

	whFiles := make([]string, 0)
	opqDirs := make([]string, 0)

	err = a.walker.Walk(layerInfo.Path, a.artifactOption.WalkerOption, func(filePath string, info os.FileInfo, opener analyzer.Opener) error {
		a.logger.Info("Walk", log.String("path", filePath), log.Any("ischar", info.Mode()&os.ModeCharDevice))
	...
	a.logger.Info("WHITEOUT", log.Any("opaque", opqDirs), log.Any("whiteout", whFiles))
...
func (a Artifact) uncompressedLayer(diffID string) (string, error) {
	a.logger.Info("LayerByDiffID", log.String("diffID", diffID))

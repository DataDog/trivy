...
func (a bunLibraryAnalyzer) PostAnalyze(ctx context.Context, input analyzer.PostAnalysisInput) (*analyzer.AnalysisResult, error) {
	...
	err := fsutils.WalkDir(input.FS, ".", required, input.Options.WalkErrCallback, func(filePath string, _ fs.DirEntry, _ io.Reader) error {
...
func (a bunLibraryAnalyzer) findLicenses(fsys fs.FS, lockPath string, errCallback func(filePath string, err error) error) (map[string][]string, error) {
	...
	licenses := make(map[string][]string)
	err := fsutils.WalkDir(fsys, root, required, errCallback, func(filePath string, _ fs.DirEntry, r io.Reader) error {

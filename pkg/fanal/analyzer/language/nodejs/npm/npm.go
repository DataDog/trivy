...
func (a npmLibraryAnalyzer) PostAnalyze(ctx context.Context, input analyzer.PostAnalysisInput) (*analyzer.AnalysisResult, error) {
	...
	err := fsutils.WalkDir(ctx, input.FS, ".", required, input.Options.WalkErrCallback, func(filePath string, _ fs.DirEntry, _ io.Reader) error {
		// Find all licenses from package.json files under node_modules dirs
		licenses, err := a.findLicenses(ctx, input.FS, filePath, input.Options.WalkErrCallback)
		if err != nil {
...
func (a npmLibraryAnalyzer) findLicenses(ctx context.Context, fsys fs.FS, lockPath string, errCallback func(filePath string, err error) error) (map[string][]string, error) {
	...
	licenses := make(map[string][]string)
	err := fsutils.WalkDir(ctx, fsys, root, required, errCallback, func(filePath string, _ fs.DirEntry, r io.Reader) error {
